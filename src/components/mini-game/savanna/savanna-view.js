...
const view = {
  changeLevelDifficulty() {
    ...
    const content = `<span>level: ${localStorage.getItem('savanna-level')} ; difficulty:${localStorage.getItem('savanna-difficulty')}</span>`;
    document.getElementsByClassName('level-difficulty')[0].insertAdjacentHTML('afterbegin', content);
  },
  gameResult() {
